...
def break_into_list_of_words(string):
    """takes one long string and returns a list of all of the words in the string,
    ...
    while '' in list_of_words:
        list_of_words.remove('')
    # removing a lot of unnecessary punctuation, leaving only words
    for index in range(0, len(list_of_words)):
        for punc in ";\"'.,?!'": # yes, I know
            list_of_words[index] = list_of_words[index].strip(punc)

    # all we have now is a list with words without punctuation
    # (secretly, some words still have apostrophes and dashes in 'em but we don't care)
...
def count_how_many_words(word_list, counting_string):
    """takes in a LIST and a STRING and returns the number of times that string occurs in the list"""
